...
Zweryfikowano 13 twierdzeń strukturalnych, na których stoi ranking i klasyfikacja powyżej: liczby pól/metod, wzorce "liczy raz, ale w innym miejscu przelicza ponownie", liczność call-site'ów oraz pary lustrzanych typów. Metoda: `ast-grep 0.45.3` dla każdego wzorca strukturalnego; każdy wynik zerowy z ast-grep potwierdzony osobnym klasycznym `grep`. Żaden wynik nie podważa pozycji żadnego z trzech rankowanych kandydatów (C1, C4, C9) — wszystkie ich twierdzenia potwierdzone dokładnie lub z drobną korektą numeru linii. Dwie korekty dotyczą kandydatów już odrzuconych z rankingu (C6) i dodatkowo *wzmacniają* uzasadnienie odrzucenia (Program.cs jest mniejszy/mniej rozdrobniony niż raport sugerował), więc nic tu nie wymaga adnotacji "do decyzji na etapie planowania".

| # | Twierdzenie | Werdykt | Dowód (plik:linia) | Metoda (wzorzec/reguła) |
|---|---|---|---|---|
| 1 | `RouteResult.cs` ma 3 pola konstruktora + 5 computed properties (8 razem); `route.ts` lustrzanie odzwierciedla wszystkie 8 pól interfejsu `RouteResult` | **doprecyzowane** (liczba pól ok, zakres linii błędny) | `RouteResult.cs:3-13`; `route.ts:17-26 (raport: 1-26)` — linie 1-16 to sąsiednie typy (`RouteCoordinate`/`RouteWaySegment`/`RouteGeometry`), nie sam interfejs `RouteResult` | Read + ręczne porównanie pól (ast-grep nie rozróżnia TS `interface` members bez dedykowanej reguły) |
| 2 | Dokładnie 3 miejsca niesprawdzonego rzutowania `res.json() as RouteResult`(-podobne typy) | **potwierdzone** | `routingApi.ts:13`, `routingApi.ts:43`, `RouteApp.tsx:46` | `ast-grep -p 'res.json() as $TYPE'` (osobno per plik) |
| 3 | Zero użyć zod/ajv/io-ts w `src/frontend` | **potwierdzone (zero)** | brak w `package.json` i w źródłach — zero trafień | `ast-grep -p "import $$$ from 'zod'"` (0 wyników) + `grep -rn "from 'zod'\|'ajv'\|'io-ts'"` (exit 1, potwierdza zero) |
| 4 | `ComputeSharpTurnFlags` wywoływane w dokładnie 3 niezależnych miejscach (`ToMetrics` raz + `RouteResult.SmoothnessScore` + `RouteResult.MaxConsecutiveSharpTurns` każde przelicza od nowa) | **potwierdzone** | `LoopRouteGenerator.cs:80`; `SmoothnessCalculator.cs:5`; `SpikeDetector.cs:11` | `ast-grep -p 'ComputeSharpTurnFlags($$$ARGS)'` (gołe wywołanie, 1 trafienie) + `ast-grep -p 'SmoothnessCalculator.ComputeSharpTurnFlags($$$ARGS)'` (kwalifikowane, 2 trafienia) — dwa wzorce potrzebne, bo ast-grep rozróżnia wywołanie gołe od kwalifikowanego jako różne węzły AST |
| 5 | `RouteMetadataValidation.Validate` używane w dokładnie 2 miejscach produkcyjnych | **potwierdzone** | `Program.cs:185`, `Program.cs:261` | `ast-grep -p 'RouteMetadataValidation.Validate($$$ARGS)'` |
| 6 | 13 plików testowych zależy od współdzielonych typów `TestInfrastructure.cs` | **potwierdzone** | 13 plików w `VeloRoute.Tests/Routing/` (m.in. `EditRouteTests.cs`, `ShareRouteTests.cs`, `RouteQualityTests.cs`, ...) | `grep -rl` po nazwach 5 współdzielonych typów, `wc -l` = 13 |
| 7 | "82-usage `WebApplicationFactory<Program>`" jako wzorzec testowy (C4/C6) | **doprecyzowane** | Literalne `WebApplicationFactory<Program>` = **4** wystąpienia; szersza fraza `WebApplicationFactory` (głównie przez podklasę `VeloRouteWebApplicationFactory`) = **82** wystąpienia — liczba 82 poprawna, ale przypisana do złego wzorca w raporcie | `ast-grep -p 'WebApplicationFactory<Program>'` (4) + `grep -rn "WebApplicationFactory"` (82) |
| 8 | Zero grupowania endpointów przez extension methods (`this IServiceCollection`/`this WebApplication`, styl `MapXEndpoints`) w backendzie | **potwierdzone (zero)** | brak w `src/backend/VeloRoute` | `ast-grep -p 'public static $RET $NAME(this IServiceCollection $$$)'` i analogicznie dla `this WebApplication` (0 wyników) + `grep -rn "static.*this IServiceCollection\|MapXEndpoints\|Endpoints(this"` (exit 1, potwierdza zero) |
| 9 | `Program.cs` ma 468 linii | **potwierdzone** | `Program.cs` | `wc -l` (fakt liczbowy, nie wzorzec AST — brak sensownego wzorca ast-grep dla "liczby linii pliku") |
| 10 | "~18 endpointów" zarejestrowanych inline w `Program.cs` | **doprecyzowane** | **14** (raport: ~18) — `Program.cs:129,139,142,153,180,209,224,241,284,302,344,364,380,416` | `ast-grep -p 'app.$METHOD($$$ARGS)'` odfiltrowane do `MapGet/MapPost/MapPatch/MapDelete/MapPut` |
| 11 | 2-argumentowy overload `GetDirectionsAsync(start, end, ...)` nie ma żadnego prawdziwego wywołania spoza własnej implementacji (dead code) | **potwierdzone** | Deklaracja: `IOpenRouteServiceClient.cs:5-8`. Jedyne "wywołania" to własna delegacja przeciążenia do wariantu listowego: `OpenRouteServiceClient.cs:23`, `TestInfrastructure.cs:81`. Jedyny zewnętrzny test (`OpenRouteServiceClientTests.cs:80`) woła wariant listowy (`waypoints: IReadOnlyList<RouteCoordinate>`), nie 2-argumentowy | `ast-grep -p '$X.GetDirectionsAsync($A, $B, $$$REST)'` — wzorzec ślepy na typy, dopasował też wariant listowy; rozstrzygnięcie wymagało odczytu sygnatur (`IOpenRouteServiceClient.cs`) i argumentów testu wprost |
| 12 | Brak endpointu `/routes/preview` w `Program.cs` | **potwierdzone (zero)** | `Program.cs` — zero trafień na "preview" | `ast-grep -p 'app.$METHOD($PATH, $$$)'` odfiltrowane po "preview" (0) + `grep -in "preview" Program.cs` (exit 1, potwierdza zero) |
| 13 | `RouteMap.tsx` ma dokładnie 3 konsumentów; `RouteApp.tsx` ma dokładnie 1 | **potwierdzone** | RouteMap: `RouteApp.tsx:11`, `my-routes/[id]/page.tsx:12`, `r/[token]/page.tsx:9` (wszystkie przez `dynamic(() => import(...))`); RouteApp: `app/page.tsx` | `grep -rn "dynamic(() => import" \| grep -i routemap"` — statyczny wzorzec `ast-grep -p "import $$$ from '$PATH'"` nie dopasował (import jest opakowany w wywołanie `dynamic()`, nie jest to top-level `import`-statement) |

**Notatka metodologiczna:** dla twierdzeń #1, #7, #10 poprawiono numer/zakres linii lub doprecyzowano opis wzorca w tekście raportu powyżej (format „X (raport: Y)"), zgodnie z zasadą śladu korekty. Sekcje "Refactor opportunities" i werdykty intencjonalności w "Per-candidate findings" pozostały bez zmian — żadna korekta ich nie podważa.

## Code References

- `src/backend/VeloRoute/Routing/RouteResult.cs:3-13` — wire contract, computed properties (C1, C2)
- `src/frontend/src/types/route.ts:17-26` (raport: 1-26) — hand-mirrored frontend contract (C1)
