...
Backend test runner: xUnit 2.9.3, bootstrapped in `src/backend/VeloRoute.Tests/`. Run with `dotnet test` from `src/backend/`. No frontend test runner yet.
...
- **Data flow**: frontend → HTTP → backend → OpenRouteService (ORS) HTTP API. Location inputs are not persisted server-side.
